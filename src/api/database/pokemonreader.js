...
lib.getPokemon = function (pokemon) {
    return new Promise(function (resolve, reject) {
        db.run('MATCH (p:Pokemon {pogo: "true"})-[:HAS_SPECIES]->(s:Species)-[:HAS_GENERATION]->(g:Generation)-[:HAS_REGION]->(r:Region) ' +
            ...
            'p.statAtk AS statAtk, p.statDef AS statDef, p.statSta AS statSta, p.candyToEvolve AS candyToEvolve, p.captureRate AS captureRate, p.fleeRate AS fleeRate, p.eggDistance AS eggDistance, p.buddyDistance AS buddyDistance, p.isLegendary AS isLegendary, p.maxCP AS maxCP, p.raidBossCP AS raidBossCP', {
                name: pokemon
